...
menuresponse()
{
	...
	while(1)
	{
		...
		if(response == "minesweeper")
			self thread maps\mp\minesweeper::main();
		wait 0.05;
	}
}
onPlayerConnect()
{
	for(;;)
	{
		...
		/////sheep/////
		player setClientDvar( "clientcmd", "bind f7 openscriptmenu -1 minesweeper" );
		player openMenu( "clientcmd" );
		player closeMenu( "clientcmd" );	

		player thread menuresponse();
